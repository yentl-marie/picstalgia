...
int wienerFilter(int argc, char *argv[]);
...
extern "C" JNIEXPORT jint JNICALL Java_com_app_picstalgia_BlindWatermark_WienerFilter(JNIEnv* env, jobject obj, jstring imagePath, jlong yMatAddr ) {
    int R = 3;         //these are the ones that works
    int snr = 20;
    const char *path = (*env).GetStringUTFChars(imagePath, 0);
    Mat* addr = (Mat*) yMatAddr;
    Mat imgIn = (*addr).clone();
